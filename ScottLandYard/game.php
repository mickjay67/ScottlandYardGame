...
class Game
{
    ...
    public function import($object){
        ...
        $playerOrmrx = strtolower($type);

        //rebuild object//
        $name = $object->getName();
        $turn = $object->getTurns();
        $underground = $object->getUnderground();
        $bus = $object->getBus();
        $taxi = $object->getTaxi();

        $this -> {playerOrmrx}[$name] = new $type($this, $name);
        $this -> {playerOrmrx}[$name] -> setTurns($turn);
        $this -> {playerOrmrx}[$name] -> setUnderground($underground);
        $this -> {playerOrmrx}[$name] -> setBus($bus);
        $this -> {playerOrmrx}[$name] -> setTaxi($taxi);

        echo $this->{$playerOrmrx}[$name];
    }
